...
def _sweetalert_gorunur_metin(page: Page) -> str:
    """GÖRÜNÜR SweetAlert metni ('' yoksa) — `_sweetalert_text`'in ÇALIŞAN görünürlük testli ikizi.
    🔴 CANLI ÖLÇÜM (2026-07-16, BV/AHMET HAKAN OĞUZ): `.sweet-alert` **`position:fixed`** → `offsetParent`
    SPEC GEREĞİ DAİMA null (popup GÖRÜNÜRKEN bile!) → `_sweetalert_text`'in `offsetParent!==null` filtresi
    hep boş döndürüyor ⇒ 'pdks kaydı mevcuttur' reddetme popup'ı HİÇ yakalanmamış (motor generic
    'Dialog Kaydet tutmadı' basıyordu). `display` + `offsetWidth` fixed'den ETKİLENMEZ → doğru test bunlar.
    ⚠ `_sweetalert_text` BİLEREK DÜZELTİLMEDİ: onu çalışır yapmak BAŞARI popup'larını da yakalayıp
    `_dialog_save`'in happy-path'inde sahte hata üretebilirdi (canlı-kanıtlanmış PRIME davranışı korunur).
    Bu ikiz yalnız 'hatırlatma' amaçlı, akışı DEĞİŞTİRMEDEN kullanılır."""
    return page.evaluate(
        r"""() => {const s=[...document.querySelectorAll('.sweet-alert')].filter(e=>{
              const cs=getComputedStyle(e);
              return cs.display!=='none' && cs.visibility!=='hidden' && (e.offsetWidth>0||e.offsetHeight>0);})[0];
            return s ? s.innerText.replace(/\s+/g,' ').trim() : '';}""")
...
def _ekrani_temizle(page: Page):
    """Kişi PATLADIKTAN sonra ekranı temizle: reddetme popup'ına TAMAM + yarım ✎ dialogunu kapat.
    🔴 CANLI BULGU (2026-07-16, BV — kullanıcı yakaladı): 'pdks kaydı mevcuttur' popup'ı ekranda KALIYOR.
    `reopen_fresh_form` yalnız FORMU kapatır (o da evaluate-click → overlay'i delip geçer), SweetAlert'e
    DOKUNMAZ → kalan overlay sonraki kişinin GERÇEK tıklamalarını yer → zincirleme başarısızlık
    (AHMET HAKAN OĞUZ'dan sonra SUNA/ÖNDER'in aynı hatayı yemesi bununla açıklanıyor).
    Ekran görüntüsü ALINDIKTAN SONRA çağrılır (kanıt korunur). Asla exception sızdırmaz — temizlik
    asıl hatayı maskelememeli."""
    try:
        _dismiss_sweetalert(page)
    except Exception:
        pass
    try:
        _close_dialog_hard(page)   # yalnız ✎ alt-dialogu (span.edit-gird-close); formu SIFIRLAMAZ (Escape/X ASLA)
    except Exception:
        pass
...
def _dialog_save(page: Page):
    ...
    wait_for_form_idle(page)
    gorulen_sa = ""   # bu denemede GÖRÜLEN son reddetme popup'ı: BV'de bir an belirip kayboluyor (2026-07-16)
    for i in range(5):
        ...
        for _ in range(24):   # ~2.4 sn tavan: kapandı mı / SweetAlert çıktı mı poll (100ms granül → kapanmayı erken yakalar)
            page.wait_for_timeout(100)
            # SADECE HATIRLA — break koşulu AYNEN eski (akış değişmez → happy-path/mükerrer riski YOK).
            # Kaydet başarılıysa dialog kapanır → aşağıda `return` edilir → gorulen_sa hiç kullanılmaz,
            # yani olası BAŞARI popup'ı asla sahte hataya dönüşemez. Yalnız 'dialog hâlâ açık' (=kayıt
            # OLMADI) durumunda gerçek reddetme sebebi olarak raporlanır.
            gorulen_sa = _sweetalert_gorunur_metin(page) or gorulen_sa
    ...
    if _dialog_open(page):
        if gorulen_sa:   # popup çıkmıştı ama kayboldu → gerçek sebebi bildir ('pdks kaydı mevcut' → KALICI sınıfı, boşuna retry yok)
            _dismiss_sweetalert(page); _close_dialog_hard(page)
            raise VerifyError(f"Dialog Kaydet reddedildi (portal): {gorulen_sa[:80]}")
        ...
        try:
            ...
            for i, person in enumerate(targets, 1):
                ...
                try:
                    ...
                    _ekrani_temizle(page)   # popup/overlay ekranda kalmasın → sonraki kişi bloklanmasın (2026-07-16 BV)
