...
def parse_report(txt: str) -> list[tuple[str, float | None, str]]:
    """KOLON-KAYMASI TOLERANSLI parse (verify_sgk.py'den kanıtlanmış): her TC ('*'li) satırında
    ilk HH:MM kolonundan ÖNCEKİ son iki 'XX,XX' = (Gelir Vergi İstisnası Gün, Sigorta Primi Gün).
    Dönüş: [(ad, gv_gun, sig_str), ...]."""
    rows = list(csv.reader(io.StringIO(txt), delimiter=';'))
    out = []
    for r in rows:
        if not r or '*' not in r[0]:        # TC (maskeli) satırı değil → atla
            continue
        ad = next((c.strip() for c in r[1:12] if c.strip() and not _NUM.match(c.strip())
                   and not re.match(r'^\d', c.strip())), '?')
        ti = next((i for i, c in enumerate(r) if _TIME.match(c.strip())), len(r))
        xx = [c.strip() for c in r[:ti] if _NUM.match(c.strip())]
        gv = xx[-2] if len(xx) >= 2 else None
        sig = xx[-1] if len(xx) >= 1 else '?'
        out.append((ad, _num(gv) if gv else None, sig))
    return out
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            sys.exit(2)
        csv_path = f"/tmp/sgk_{args.lokasyon}_{args.sheet}.csv"
    ...
    if not people:
        ...
        sys.exit(2)
    in_report = {fold(ad) for ad, _, _ in people}
    short = [(ad, gv, sig) for ad, gv, sig in people if gv is not None and gv < args.threshold]
    full = [p for p in people if p[2] != '?' and (p[1] is None or p[1] >= args.threshold)]

    # ÇAPRAZ-REFERANS
    our_short = [(ad, gv, sig) for ad, gv, sig in short if fold(ad) in our]
    other_short = [(ad, gv, sig) for ad, gv, sig in short if fold(ad) not in our]
    our_missing = sorted(fold2orig[f] for f in our if f not in in_report)

    print()
    log(f"==== RAPOR: {len(people)} kişi | Gün=30 (tam): {len(people)-len(short)} | Gün<30: {len(short)} ====")
    print()
    log(f">>> BİZİM EKSİK/HATALI ({len(our_short)}) — Gün<30, DÜZELTİLMELİ:")
    for ad, gv, sig in sorted(our_short, key=lambda x: x[1] if x[1] is not None else 0):
        log(f"     {ad[:34]:34} | GV Gün={gv:>6} | Sig={sig:>6}")
    if not our_short:
        log("     (yok — bizim girdiğimiz herkes Gün=30 ✓)")
    print()
    ...
    print()
    log(f">>> KAPSAM DIŞI Gün<30 ({len(other_short)}) — bizim-olmayan (Destek/ayrılan?), yalnız bilgi:")
    for ad, gv, sig in other_short[:40]:
        log(f"     {ad[:34]:34} | GV Gün={gv:>6} | Sig={sig:>6}")

    fix_targets = sorted(set([fold2orig[fold(ad)] for ad, _, _ in our_short if fold(ad) in fold2orig] + our_missing))
    print()
    if fix_targets:
        log(f"==== DÜZELTİLECEK (bizim): {len(fix_targets)} kişi ====")
        ...
        log("==== ✅ TEMİZ: bizim girdiğimiz herkes Gün=30. Düzeltme gerekmez. ====")
