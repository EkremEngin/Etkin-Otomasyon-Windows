...
# 🔑 T.C.-BAZLI KİMLİK (2026-07-09 kullanıcı fikri): dropdown maskeli T.C. gösterir ("GAMZE AYTEKİN 25*******28").
# Excel'in AÇIK T.C.'sinin ilk2+son2'si maskeyle eşleşir → isim evlilik/kızlık farkına DAYANIKLI. args=[tc11, tokens].
...
_PICK_LI_BY_TC_JS = r"""(args) => {
  ...
  for(const t of ['mouseover','mouseenter','mousemove','mousedown','mouseup','click'])
    tgt.dispatchEvent(new MouseEvent(t,{bubbles:true,cancelable:true,view:window}));
  return JSON.stringify({ok:true, picked:li.textContent.trim(), overlap:best.ov, multi:pick.length});
}"""
...
def _click_combo(page: Page) -> bool:
    """▼ 'Tümünü Göster' combo toggle'ına GERÇEK Playwright click. Klavye autocomplete-search bazı isimlerde
    dropdown'ı TETİKLEMİYOR (2026-07-09 kullanıcı bulgusu — BV: Ghaith/Gamze/Gizem/Steffen; TPI: 6 straggler
    hep 'öneri çıkmadı' bundandı, FATIGUE DEĞİL). Butona basınca menü açılır → mevcut token-eşleşme seçer.
    DOM: <input name=string_Personel_Id> KARDEŞİ <a class="nothover combo" title="Tümünü Göster">."""
    try:
...
def select_personel(page: Page, portal_ad: str, tc: str | None = None):
    """Personel seç + gizli Personel_Id + grid'i DOĞRULA. KİMLİK = maskeli T.C. (2026-07-09): Excel'in AÇIK
    T.C.'sinin ilk2+son2'si dropdown'daki maskeli T.C. ile eşleşir → isim evlilik/kızlık farkına DAYANIKLI
    (portal 'GAMZE AYTEKİN', Excel 'Gamze Aytekin Makam' = aynı T.C.). İsim örtüşmesiyle ÇİFT-KONTROL. Arama
    soyad→ilk-ad→tam-ad, her denemede ▼ combo fallback; widget bağlanana + grid dolana dek 3 kez dene."""
    import json
    ...
    _terms = [surname, first, portal_ad]
    last_err = None
    for attempt in range(3):
        ...
        li_ok = False
        try:
            page.wait_for_function(_TARGET_VISIBLE_JS, arg=[tc_arg, tokens], timeout=4000)
            li_ok = True
        except PWTimeout:
            # ▼ FALLBACK (2026-07-09 kullanıcı bulgusu): klavye-search dropdown'ı tetiklemedi →
            # 'Tümünü Göster' combo butonuna GERÇEK tıkla; menü açılınca T.C./token eşleşmesi seçer.
            if _click_combo(page):
                try:
                    page.wait_for_function(_TARGET_VISIBLE_JS, arg=[tc_arg, tokens], timeout=6000)
                    li_ok = True
                except PWTimeout:
                    pass
        if not li_ok:
            # TEŞHİS: hedef çıkmadıysa menüde NE VAR logla (kullanıcı isteği: ne döndüğünü anla)
            shown = page.evaluate(r"""()=>[...document.querySelectorAll('ul.ui-autocomplete li,ul.ui-menu li')].filter(x=>x.offsetParent!==null).map(x=>x.textContent.trim()).slice(0,8)""")
        ...
        if not pick.get("ok"):
            ...
            _clear_personel(page); page.wait_for_timeout(800); continue
        if use_tc and pick.get("overlap", 0) < len(tokens):
            log(f"  (T.C.-eşleşme: portal '{pick.get('picked')}' ↔ Excel '{portal_ad}' T.C.{tcs[:2]}..{tcs[-2:]}"
                f" — isim farkı=evlilik/kızlık, T.C. teyitli)")
...
def process_one_person(page: Page, person: IzinPerson, first: bool) -> dict:
    ...
    select_personel(page, person.portal_ad, getattr(person, "tc", None))
    grid = read_grid(page)
