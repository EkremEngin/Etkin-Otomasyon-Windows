...
==============================================================================
Onayı PDF gerektiren parklarda (İYTE, YTP) her personelin izin belgesini bulur ve kişiyle eşleştirir.
Kaynak klasör iki yoldan gelir (ikisi de sadece bir PATH — ileride exe'nin "dosya yükle" arayüzü de
aynı path'i verecek):
  ...
  2) Açık verilen klasör: `--belge-klasor /yol/...`

EŞLEŞTİRME (iki güvenli yol, ikisi de yanlış-pozitif vermez):
...
def readiness(people, folder: str | None) -> dict:
    """Kişi listesi + klasör → {matched{tc:[dosya]}, missing[person], extra[dosya], nonpdf[dosya], folder}."""
    matched, missing = {}, []
    used = set()
    for p in people:
...
def report(park, people, base: str | None = None, folder: str | None = None) -> dict:
    """Parkın belge hazırlık raporunu bas + readiness dict döndür."""
    folder = folder or find_belge_klasor(park, base)
    r = readiness(people, folder)
    print(f"\n{LOG} {park.code} ({park.ad}) — {r['toplam']} kişi")
    if not folder:
...
def _main():
    ...
    ap.add_argument("--park", required=True, help="PDF'li park (İYTE/YTP)")
    ...
    r = report(pk, people, base=args.base, folder=args.belge_klasor)
