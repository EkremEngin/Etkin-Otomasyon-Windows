...
def run_approval_pdfli(park: Park, meta: dict, people, belge_klasor: str | None, cdp: str, limit: int):
    """PDF'li parklar (İYTE/YTP): belgeleri çöz → HEPSİ hazırsa kanıtlı yükleyiciyi (subprocess) çalıştır.
    ADAPTİF: belge eksikse onaya GÖNDERMEZ (giriş taslak kalır), net rapor verir — İYTE/YTP'yi beklemeden
    ...
    r = izin_belge.report(park, people, folder=folder)
    if r["eslesen"] != r["toplam"] or r["nonpdf"]:
        log(f"[ONAY] {park.code}: belgeler TAM DEĞİL ({r['eslesen']}/{r['toplam']}"
            f"{', PDF-dışı var' if r['nonpdf'] else ''}) → onaya GÖNDERİLMEDİ (adaptif). "
            f"Giriş TASLAK kaldı; eksik belgeleri koyup tekrar `--onayla` çalıştır.")
        return
    script = os.path.join(SCRIPT_DIR, "izin_onaya_dosyali_yildiz.py")
    cmd = [sys.executable, script, "--lokasyon", park.label, "--sheet", ay_key,
           "--donem", meta["donem_label"], "--dosya-klasor", folder]
    if limit:
        cmd += ["--limit", str(limit)]
    env = os.environ.copy()
    env["DGS_CDP"] = cdp
    log(f"\n[ONAY] {park.code}: {r['toplam']} belge hazır ✓ → yükle+onaya gönder (makinalı tüfek):")
...
def run_plan(excel: str, parklar, belge_klasor: str | None, detay: bool = True):
    ...
    for code in order:
        ...
        if pk.onay_pdf:
            ...
            rd = izin_belge.readiness(people, folder)
            hazir = folder and rd["eslesen"] == rd["toplam"] and not rd["nonpdf"]
            durum = f"belge {rd['eslesen']}/{rd['toplam']}" + (" HAZIR ✓" if hazir else
                     (" ⚠ klasör YOK" if not folder else " ⚠ EKSİK"))
            pdf = f"  [ONAY: PDF gerekli — {durum}]"
        else:
...
def main():
    ...
    if args.plan:
        run_plan(excel, parklar, args.belge_klasor)
        return
