...
def approve_drafts(page, donem_label, our, onay_file, limit=10**9, already=None, log=log, our_tc=None):
    ...
    FIND_FIRST = (r"""(args)=>{const [our, failed, ourTc]=args; %s
      for(const tr of document.querySelectorAll('tr')){
        ...
        if(!our.includes(nf) && !ourTc.includes(tcKey)) continue;   // GÜVENLİK: bizim (isim VEYA maskeli-T.C.)
    ...
    for _iter in range(len(our) + 25):
        ...
        if not list_open():        # liste kapandıysa yeniden aç (filtre+dönem ile)
            open_list()
        listele()
        name = page.evaluate(FIND_FIRST, [list(our), list(failed), list(our_tc or [])])
        if not name:
